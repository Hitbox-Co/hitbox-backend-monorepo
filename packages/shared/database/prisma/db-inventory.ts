...
async function main() {
    const [products, users, artists, claims, ledger] = await Promise.all([
        prisma.drop.findMany({ select: { id: true, productCode: true, name: true, tagId: true, claimedStatus: true } }),
        prisma.user.findMany({ select: { id: true, username: true, email: true, clerkUserId: true } }),
        prisma.artist.count(),
        prisma.skuClaim.count(),
    ...
    console.log(`\nartists=${artists}  skuClaims=${claims}  ledgerRows=${ledger}`);
}
